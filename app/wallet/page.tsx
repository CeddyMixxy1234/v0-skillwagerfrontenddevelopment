...
export default function WalletPage() {
  ...
  const handleDeposit = async () => {
    if (!depositAmount || Number.parseFloat(depositAmount) < 5) {
      alert("Minimum deposit is $5")
      return
    }
    if (Number.parseFloat(depositAmount) > 10000) {
      alert("Maximum deposit is $10,000")
      return
    }
  ...
  const handleWithdraw = async () => {
    ...
    if (Number.parseFloat(withdrawAmount) > 250.5) {
      alert("You cannot withdraw more than your available balance ($250.50)")
      return
    }
    if (Number.parseFloat(withdrawAmount) < 10) {
