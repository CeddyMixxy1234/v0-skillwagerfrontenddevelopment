      ...
      <main className="max-w-6xl mx-auto px-3 py-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{filteredMatches.length} matches</p>
          </div>
          <Link href="/wager/browse">
            <Button className="bg-primary text-primary-foreground text-sm py-1 px-2 h-8">Browse Wagers</Button>
          </Link>
        </div>
          ...
          <div className="flex gap-1 flex-wrap">
            {(["all", "live", "completed", "disputed"] as const).map((status) => (
              <Button
                ...
                className={`text-xs py-0.5 px-1.5 h-7 ${filterStatus === status ? "bg-primary" : "bg-transparent"}`}
              >
        ...
        {filteredMatches.length > 0 ? (
          <div className="space-y-2">
            {filteredMatches.map((match) => (
              <Link key={match.id} href={`/matches/${match.id}`}>
                <Card className="p-3 border-border hover:border-primary/30 transition-colors cursor-pointer">
                  <div className="grid grid-cols-1 md:grid-cols-6 gap-2 items-center text-sm">
                    {/* Game & Opponent */}
                    <div className="md:col-span-2 flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-sm">{match.game}</p>
                        <p className="text-xs text-muted-foreground truncate">vs {match.opponent}</p>
                        <p className="text-xs text-muted-foreground">{match.mode}</p>
                      </div>
                    </div>

                    {/* Wager & Status */}
                    <div>
                      <p className="text-xs text-muted-foreground mb-0.5">Wager</p>
                      <p className="text-sm font-bold text-accent">${match.wagerAmount}</p>
                      <div
                        className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded border text-xs font-medium mt-1 w-fit ${getStatusColor(match.status)}`}
                      >
                    ...
                    <div className="text-center">
                      <p className="text-xs text-muted-foreground mb-0.5">Your Stats</p>
                      {match.status === "live" && (
                        <div className="space-y-0.5">
                          {match.yourStats && "kills" in match.yourStats && (
                            <>
                              <p className="text-xs font-semibold">{match.yourStats.kills} Kills</p>
                              <p className="text-xs text-muted-foreground">Placement: #{match.yourStats.placement}</p>
                            </>
                          )}
                        </div>
                      )}
                      {match.status === "completed" && match.result !== "disputed" && (
                        <div className="space-y-0.5">
                          ...
                          {match.yourStats && "movesPlayed" in match.yourStats && (
                            <p className="text-xs font-semibold">{match.yourStats.movesPlayed} Moves</p>
                          )}
                        </div>
                      )}
                    </div>
                      ...
                      {match.status === "live" && (
                        <div className="space-y-0.5">
                          {match.opponentStats && "kills" in match.opponentStats && (
                            <>
                              <p className="text-xs font-semibold">{match.opponentStats.kills} Kills</p>
                      ...
                      {match.status === "completed" && match.result !== "disputed" && (
                        <div className="space-y-0.5">
                          ...
                          {match.opponentStats && "movesPlayed" in match.opponentStats && (
                            <p className="text-xs font-semibold">{match.opponentStats.movesPlayed} Moves</p>
                          )}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="text-right">
                      {match.status === "live" && (
                        <p className="text-xs text-muted-foreground">{match.timeRemaining} left</p>
                      )}
                      {match.status === "completed" && (
                        <div className="space-y-1">
                          {getResultBadge(match.result)}
                          {match.earnings && <p className="text-xs font-semibold text-green-500">+${match.earnings}</p>}
                        </div>
                      )}
                      {match.status === "disputed" && (
                        <Button
                          size="sm"
                          variant="outline"
                          className="bg-transparent text-xs py-0.5 px-1.5 h-6"
                          onClick={() => {
                            alert(
          ...
          <Card className="p-8 text-center border-border">
            ...
            <Link href="/wager/browse" className="text-xs text-primary hover:underline">
